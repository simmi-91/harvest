                            ...
                            <div className={`transition-opacity ${isDone ? "opacity-50" : ""}`}>
                                ...
                                <div className="sm:hidden">
                                    <div className="flex items-center gap-2 px-3 py-1.5">
                                        <div className="flex-1 min-w-0 flex items-center gap-1.5 flex-wrap">
                                            <PlantIcon category={harvest.plant_category} onClick={() => setInfoPlantId(harvest.plant_id)} />
                                            <a
                                    ...
                                    {(harvest.amount ||
                                        ...
                                        <div className="px-3 pb-1.5 text-xs text-zinc-700 space-y-0.5">
                                            ...
                                            {harvest.locations.map((loc) => {
                                                ...
                                                if (loc.position)
                                                    label.push(
                                                        formatPosition(loc.address, loc.position)
                                                    );
                                                if (loc.boxes?.length)
                                ...
                                <div
                                    ...
                                    <div className="px-3 py-1.5 text-sm text-zinc-800">
                                        {harvest.amount ?? <span className="text-zinc-400">–</span>}
                                    </div>

                                    {/* Locations + note */}
                                    <div
                                        className="px-3 py-1.5 border-l border-dashed"
                                        style={{ borderColor: "var(--color3)" }}>
                                        {harvest.locations.length > 0 ? (
                                            <ul className="space-y-0.5">
                                                {harvest.locations.map((loc) => {
                                                    const label: string[] = [];
                                                    if (loc.position)
                                                        label.push(`Tak ${loc.position}`);
                                                    else if (loc.address) label.push(loc.address);
                                                    if (loc.boxes?.length)
                                                        label.push(`kasse ${loc.boxes.join(", ")}`);
                                                    if (loc.location_note)
                                                        label.push(loc.location_note);
